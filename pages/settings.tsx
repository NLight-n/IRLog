...
// --- AdminTab Component ---
function AdminTab({ user, onUpdateAppHeading }: { user: unknown, onUpdateAppHeading: (heading: string, subheading: string) => void }) {
  ...
  React.useEffect(() => {
    // Fetch settings from backend (if implemented)
    fetch('/api/settings')
      .then(res => res.json())
      .then(data => {
  ...
  return (
    <div className="mb-8">
      ...
      <div className="general-settings-grid">
        ...
        <div className="mb-2">
          <label className="block font-medium mb-1">Save System Settings</label>
...
export default function SettingsPage() {
  ...
  const { appHeading, setAppHeading, appSubheading, setAppSubheading, refreshSettings } = useAppSettings();
  ...
  return (
    <div className={`min-h-screen bg-gray-50${theme === 'dark' ? ' dark' : ''}`}>
      <NavBar ref={navbarRef} user={session?.user || null} theme={theme} onToggleTheme={onToggleTheme} appHeading={appHeading} appSubheading={appSubheading} />
        ...
        <div className="card">
          <div className="card-body">
            ...
            {tab === 'user-management' && isAdmin && (
              <>
                <AdminTab user={session?.user} onUpdateAppHeading={handleUpdateAppHeading} />
